defmodule QuoteAssist.Tenants.Tenant do
  ...
  *not* settable here, so neither can be forged by a future HTTP caller:
  ...
  # A fully-qualified domain (e.g. `quotes.acme.com`): one or more dot-separated
  # labels then a 2+ letter TLD. Bounded length; lowercase (the changeset normalises).
  @custom_domain_format ~r/^(?=.{1,253}$)([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}$/

  @doc """
  Changeset for a tenant **requesting** a custom domain (R10-domain). Stores the
  normalised domain, moves it to `pending`, and records the DNS verification `token`
  (generated by the context). The domain stays unverified — and so unresolvable — until
  `verify_custom_domain_changeset/1` runs after a real DNS check. A tenant cannot claim a
  host on the platform's own base domain (that's its subdomain, not a custom domain).
  """
  def custom_domain_changeset(tenant, attrs, base_host) do
    tenant
    |> cast(attrs, [:custom_domain])
    |> update_change(:custom_domain, &normalize_host_part/1)
    |> validate_required([:custom_domain])
    |> validate_format(:custom_domain, @custom_domain_format, message: "must be a valid domain")
    |> validate_not_base_domain(base_host)
    |> put_change(:custom_domain_status, :pending)
    |> put_change(
      :custom_domain_token,
      attrs[:custom_domain_token] || attrs["custom_domain_token"]
    )
    |> unique_constraint(:custom_domain, name: :tenants_custom_domain_live_index)
  end

  @doc "Marks the custom domain verified (R10-domain) — set only after a passing DNS check."
  def verify_custom_domain_changeset(tenant) do
    change(tenant, custom_domain_status: :verified)
  end

  @doc "Clears the custom domain back to `none` (R10-domain), dropping the token."
  def clear_custom_domain_changeset(tenant) do
    change(tenant, custom_domain: nil, custom_domain_status: :none, custom_domain_token: nil)
  end

  # Reject a custom domain that is the platform base host or any subdomain of it — those
  # are reached via the slug, never as a "custom" domain, and allowing it would let a
  # tenant shadow the platform.
  defp validate_not_base_domain(changeset, base_host) when is_binary(base_host) do
    validate_change(changeset, :custom_domain, fn :custom_domain, domain ->
      if domain == base_host or String.ends_with?(domain, "." <> base_host),
        do: [custom_domain: "can't be on #{base_host}"],
        else: []
    end)
  end

  defp validate_not_base_domain(changeset, _base_host), do: changeset
