defmodule QuoteAssistWeb.UserAuth do
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      QuoteAssistWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule QuoteAssistWeb.PageLive do
        use QuoteAssistWeb, :live_view

        on_mount {QuoteAssistWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{QuoteAssistWeb.UserAuth, :require_authenticated}] do
  ...
  # Land signed-in users in the app workspace. Tenant scoping + the real shell
  # arrive in R2; for now /app is a minimal authenticated placeholder.
